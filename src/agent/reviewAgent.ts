...
function uniqueEvidenceBySource(evidence: EvidenceItem[]): EvidenceItem[] {
...
function buildPacket(request: ReviewRequest, context: AnomalyContext): AdvisoryPacketDraft {
  ...
  const supportingEvidence = uniqueEvidenceBySource(context.evidence);
  const independence = analyzeEvidenceIndependence(supportingEvidence);
  ...
  const sourceIds = [...new Set(supportingEvidence.map(item => item.source_id))];
  const contradiction = new Set(supportingEvidence.map(item => item.assessment).filter(Boolean)).has('normal')
    && new Set(supportingEvidence.map(item => item.assessment).filter(Boolean)).has('elevated');
  const uncertainty = [
    'Synthetic evidence only; root cause is not established.',
    ...supportingEvidence.flatMap(item => item.uncertainty_notes)
  ...
  return {
    ...
    evidence_summary: supportingEvidence.map(item => `${item.source_id}: ${item.excerpt}`).join(' | '),
    source_ids: sourceIds,
    provenance: 'Evidence retrieved from the local synthetic PRAETOR MCP dataset by the bounded review agent.',
    supporting_evidence: supportingEvidence,
    confidence: Math.min(0.49, Math.max(0.1, supportingEvidence.length * 0.1)),
    uncertainty,
...
export class ReviewAgent {
  ...
  async buildAndSubmit(request: ReviewRequest): Promise<ReviewResult | ReviewBlockedResult> {
    ...
    const comparisonResult = compareEvidence(packet.supporting_evidence.map(item => ({
      ...item,
      source_type: 'MCP_RETRIEVED'
    })));
